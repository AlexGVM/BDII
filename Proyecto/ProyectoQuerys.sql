...
insert into USUARIO 
(USUARIO,NOMBRE1,NOMBRE2,APELLIDO1,APELLIDO2,EMAIL,FECHADENACIMIENTO,FECHACREACIÓN, LIMITEAMIGOS)
VALUES('alexv','Alex','Magan','Juan','Muñoz','edparedes50@ucoz.ru','8/11/1994','8/11/2020', 50)
...
as
	...
	insert into USUARIO 
	(USUARIO,NOMBRE1,APELLIDO1,APELLIDO2,EMAIL,FECHADENACIMIENTO,FECHACREACIÓN,LIMITEAMIGOS)
	VALUES('papsfsdfi-kun','Josdfel','Swisdfndle','Twisdfddle','mmatusovsky0@cdc.gov','8/11/2016','2020/09/23', 50)
...
begin
	...
	where @typelikeC = 1 and MONTH(@fechaPT) = MONTH(@fechaC) and YEAR(@fechaPT) = YEAR(@fechaC) and DAY(@fechaPT) = DAY(@fechaC) and (@idPostC = PT.IDUSER)
...
select IDPOST, COUNT(Typelike)
from BITACORA
where TYPELIKE = 1
group by IDPOST
having COUNT(Typelike) >= 15
...
select YEAR(P.FECHA) as Año , MONTH(P.fecha) as Mes, COUNT(*) as Cantidad, C.IDPOST
from COMENTARIOS C
	inner join  POST P on C.IDPOST = P.IDPOST
WHERE C.ACTIVO = 1
GROUP  BY YEAR(P.FECHA), MONTH(P.fecha), C.IDPOST
HAVING COUNT(*) >= 3
